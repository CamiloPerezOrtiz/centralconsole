...
#---------------------MAIN-------------------------------#        
        while True:
            ...
            ipList = open("/var/www/html/centralconsole/web/centosFiles/ipList.txt", "r")
            #'ipListNoSSH.txt': IPs which have no SSH connection
            txt_ip_SSH= open("/var/www/html/centralconsole/web/centosFiles/ipListSSH.txt", "w")  
            txt_ip_SSH.write("")
            txt_ip_no_SSH = open("/var/www/html/centralconsole/web/centosFiles/ipListNoSSH.txt", "w")  
            txt_ip_no_SSH.write("")
            i = 1
            for line in ipList:
                ...
                txt = open("/var/www/html/centralconsole/web/centosFiles/userClient.txt", "r")
                userClient = txt.read()
                temp = len(userClient)
                ...
                txt = open("/var/www/html/centralconsole/web/centosFiles/portClient.txt", "r")
                portClient = txt.read()
                temp = len(portClient)
                portClient = portClient[:temp-1]
                txt.close()
                txt = open("/var/www/html/centralconsole/web/centosFiles/pswClient.txt", "r")
                pswClient = txt.read()
                temp = len(pswClient)
                ...
                txt_ip_no_SSH = open("/var/www/html/centralconsole/web/centosFiles/ipListNoSSH.txt", "a") 
                txt_ip_SSH= open("/var/www/html/centralconsole/web/centosFiles/ipListSSH.txt", "a")   
                user = commands.getoutput("whoami")       # Server's User
                txtStatus = open("/var/www/html/centralconsole/web/centosFiles/sshStatus.txt", "w") #change to correct path
                ...
                if status[0] == 0: # return 0 whit no errors. Public key on in the Server
                    ...
                    txtStatus.write("1")
                    key_status = commands.getoutput("ssh-keygen -i /root/.ssh/id_rsa.pub -t rsa -q -P '' ; echo 'Key generated... Trying to copy to the client' ")
                    print(key_status) 
